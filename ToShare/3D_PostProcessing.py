...
wave_range=[0.3,2.5] #range MUST match range of refractive indicies
#but also picaso is weird about going shorter than 0.3 microns
#so don't slap in any refractive indicies for shorter than 0.3 microns
opacity = pdi.opannection(wave_range=wave_range)
neptune = pdi.inputs()
#Load in PT profile
...
#define spatial grid
grid_size = 2 #i use 2 for testing, 10 for normal
neptune.phase_angle(0, num_gangle=grid_size, num_tangle=grid_size) #radians
#um mm i just copy and pasted params from fact sheets 
...
nep_temp = np.tile(np.array(temp), (len(lon), len(lat), 1))
...
nwno = 23 #number of refractive indicies
#vv wavenumber grid to use for clouds. MUST exactly match the wavenumber grid of a 1D run
#this part fucked me for like a week. i ended up just copy nd pasting from the 1D run 
...
20000.        , 25000.        , 33333.33333333])

#reading in cloud info calculated previously 
#i have patchy slab clouds so i create opd, g0, and w0 arrays for each flavor of cloud (in this case just 2)
...
df_base['g0'] = df_1['g0']+df_2['g0']+df_3['g0']
df_base['w0'] = df_1['w0']+df_2['w0']+df_3['w0']
df_base['opd'] = df_1['opd']+df_2['opd']+df_3['opd']

#where everything's going into 
...
#cloud flavor 2 (dont worry about the names. its based on some other paper)
...
#define locations of clouds as indices (so the base cloud wraps around the entire planet, covers length of lon array)
lat_base = [0,64]
lon_base = [0,128]
lon_spot = [32,96] 
lat_spot = [30,34]
...
for press in range(len(pres)-1):
    for wave in range(nwno):
        ...
        test[lon_base[0]:lon_base[1], lat_base[0]:lat_base[1],press,wave] += 0

        opd_array[lon_spot[0]:lon_spot[1], lat_spot[0]:lat_spot[1],press,wave] += opd_4[press,wave]
        g0_array[lon_spot[0]:lon_spot[1], lat_spot[0]:lat_spot[1],press,wave] +=  g0_4[press,wave]
        w0_array[lon_spot[0]:lon_spot[1], lat_spot[0]:lat_spot[1],press,wave] +=  w0_4[press,wave]
        test[lon_spot[0]:lon_spot[1], lat_spot[0]:lat_spot[1],press,wave] = 1



#plot the test map if you want to see spatially where the clouds are 
...
ds = xr.Dataset(
    data_vars=dict(
        temperature=(["lon", "lat","pressure"], nep_temp,{'units': 'Kelvin'})#, required
...
#pdb.set_trace()
#ds_cld['opd'].isel(pressure=50,wno=0).plot(x='lon',y='lat'); plt.show() ->> TO SHOW CLOUD AT GIVEN PRES/WNO
